...
class WeatherChecker {
    ...
    rateLimiter = new rate_limiter_1.default(10, 60000); // 10 requests per minute
    async getWeather(location) {
        ...
        if (cached) {
            logger_1.default.info("Weather data retrieved from cache", { location });
            return cached;
        }
...
- Precipitation: ${current.precipitation} mm`;
            ...
            logger_1.default.info("Weather data fetched successfully", { location });
            return result;
        }
        catch (error) {
            logger_1.default.error("Weather fetch error", error);
            throw new Error(`Failed to get weather: ${error.message}`);
        }
    }
}
const weatherChecker = new WeatherChecker();
exports.default = weatherChecker;
